...
- У `layouts/site.blade.php` зʼявляється змінна `$theme` з дефолтом `'theme-cream'` (`@extends('layouts.site', ['theme' => $theme])`). Решта існуючих сторінок (`/`, `/products`, CMS) працює без змін — підхоплює дефолт.
...
- `product-gallery.blade.php` — thumbs стрічка + велике main img + zoom-hint. Джерело: `getMedia('gallery')` → fallback на `getFirstMedia('primary')`. Емітить `data-lightbox-images` (JSON URLs) + `data-lightbox-index="0"` на главному `<img>`.
...
- `product-pyramid.blade.php` — три рівні: `$product->notesByLevel(NoteLevel::Top)`, `…::Mid`, `…::Base`. Рівень з порожньою колекцією пропускаємо. Хедер блоку (eyebrow + h2 + опис) — статичний з перекладів.
- `product-character.blade.php` — два bar-row для `sillage_score` (1..5) і `longevity_hours` (1..12). Ширина філа = `value / max * 100%`, словесна оцінка із `__('catalogue.public.product.character.sillage_words.'.($score-1))`. Якщо `sillage_score` NULL — ховаємо тільки рядок sillage, не весь блок (так само для longevity). Якщо обидва NULL — компонент рендерить порожньо; це покривається `@if` обгорткою в `show.blade.php`.
...
- `crumb_catalog`
- `pyramid.{title, top, mid, base, subtitle}`
- `character.{sillage_label, longevity_label, sillage_words[1..5], longevity_word_h}`
...
- Render містить `<livewire:order-form` із заповненим subject
